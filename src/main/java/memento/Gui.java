...
public class Gui extends Application {
    ...
    public void start(Stage stage) {
        ...
        historyButton.setOnAction(event -> {
            ...
            List<IMemento> list = controller.getHistory();
            for (int i = 0; i < list.size(); i++) {
                int capturedIndex = i;
                Button mementoButton = new Button("State " + i);
                mementoButton.setOnAction(e -> controller.restoreToIndex(capturedIndex));
                historyBox.getChildren().add(mementoButton);
            }
            historyStage.show();
        });
